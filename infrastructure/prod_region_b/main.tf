terraform {
  backend "s3" {
    ...
    key            = "region-b/terraform.tfstate"
    region         = "ap-south-1" # State bucket is in Mumbai
...
module "load_balancer" {
  ...
  alb_sg_id         = module.security.node_sg_id
...
module "dns" {
  ...
  lb_zone_id  = "Z1L7J7O1Y9XU" # Example ALB Zone ID for ap-southeast-1
  region      = local.region
  tags        = local.tags
}
